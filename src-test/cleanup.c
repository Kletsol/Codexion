...
/*   Updated: 2026/06/22 16:14:39 by lbonnet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	destroy_coders(t_sim *sim)
{
	...
	{
		pthread_mutex_destroy(&sim->coders[i].comp_mutex);
		i++;
	}
}
...
void	destroy_simulation(t_sim *sim)
{
	...
	while (++i < sim->nb_coders)
			printf("ID : %d | Nb_compiles : %d\n", sim->coders[i].id, sim->coders[i].nb_compiles);
	if (sim->coders)
	{
		while (++i < sim->nb_coders)
